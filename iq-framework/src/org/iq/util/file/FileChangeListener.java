...
public abstract class FileChangeListener extends AbstractFileListener {
  ...
  private HashMap<String, FileProperty> filesMap = new HashMap<String, FileProperty>();
  private HashMap<String, FileChangeStatus> filesStatusMap = new HashMap<String, FileChangeStatus>();
  ...
  private class FileProperty {
	...
	private FileChangeStatus fileChangeStatus = FileChangeStatus.NO_CHANGE;
     ...
     * 
     */
	public FileProperty(Long lastModified, FileChangeStatus fileChangeStatus) {
	  this.lastModified = lastModified;
	  this.fileChangeStatus = fileChangeStatus;
	}

	/**
	 * @return the fileChangeStatus
	 */
	public FileChangeStatus getFileChangeStatus() {
	  return fileChangeStatus;
	}

	/**
	 * @param fileChangeStatus
	 *          the fileChangeStatus to set
	 */
	public void setAction(FileChangeStatus fileChangeStatus) {
	  this.fileChangeStatus = fileChangeStatus;
	}
  ...
  private void captureFileProperty() {
	...
	if (monitoredFile.isFile()) {
	  filesMap.put(monitoredFile.getAbsolutePath(), new FileProperty(new Long(
		  monitoredFile.lastModified())));
	}
	else if (monitoredFile.isDirectory()) {
	  File[] files = getFiles();
	  if (files != null && files.length > 0) {
		for (int i = 0; i < files.length; i++) {
		  filesMap.put(files[i].getAbsolutePath(), new FileProperty(new Long(
  ...
  public boolean shouldListen() {
	...
	if (monitoredFile != null
		...
		&& monitoredFile.lastModified() != filesMap.values().iterator().next().lastModified) {
	  listenFlag = true;
	  ...
	  if (files != null && files.length > 0) {
		for (int i = 0; i < files.length; i++) {
		  ...
		  FileProperty fileProperty = filesMap.get(files[i].getAbsolutePath());
  ...
  private File[] getFiles() {
	...
	if (checkDeep) {
	  if (fileFilter != null) {
		List<File> fileList = FileUtil.listAllFiles(monitoredFile, fileFilter);
