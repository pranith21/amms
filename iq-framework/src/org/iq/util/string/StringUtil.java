...
public class StringUtil extends BaseUtil {
  ...
  public static boolean isEmpty(String inputString) {
	return inputString == null ? true : inputString.trim().length() == 0 ? true
		: false;
  }
   ...
   * <code>originalLength</code>. If input string is null or empty returns the
   * input string itself.
   * 
   * @param inputStr
   *          String input
   * @param originalLength
   *          actual length of the field upto which the padding will be done if
   *          required
   * @param paddingChar
