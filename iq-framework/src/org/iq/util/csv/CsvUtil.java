...
public class CsvUtil {/*
  ...
  */
   ...
   * UtilityException(null, e); } }
   * 
   * public static CsvObject getRecord(String csvFileName, int rowNum, CsvObject
   * csvObject) throws UtilityException { try { CSVReader reader = new
   * CSVReader(new FileReader(csvFileName)); String [] nextLine; int lineNumber
   * = 0; while ((nextLine=reader.readNext()) != null && ++lineNumber<rowNum);
   * return nextLine; } catch (FileNotFoundException e) { throw new
   * UtilityException(null, e); } catch (IOException e) { throw new
   * UtilityException(null, e); } }
   * 
   * public static void main(String[] args) { try { //csv file containing data
   * // String strFile =
   * "C:\\Users\\rsaluja\\CMS_Evaluation\\Drupal_12_08_27.csv"; CSVReader reader
   * = new CSVReader(new FileReader(CSV_FILE_NAME)); String [] nextLine; int
   * lineNumber = 0; while ((nextLine = reader.readNext()) != null) {
   * lineNumber++; System.out.println("Line # " + lineNumber);
   * 
