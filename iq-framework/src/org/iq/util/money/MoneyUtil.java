...
public class MoneyUtil {
   ...
   * 
   * String amountStr = StringUtil.getStringValue("100.00"); String[]
   * amountParts = amountStr.split("\\."); Integer amountInt =
   * Integer.valueOf(amountParts[0] + StringUtil.padCharTrail(amountParts[1], 2,
   * "0") .substring(0, 2)); Money money = new Money();
   * money.setAmount(amountInt);
   * 
   * System.out.println(getMoneyInWords(money)); }
   */
	...
	if (wholePart > 0) {
	  ...
	  while (wholePart > 0) {
		if (wholePart >= 10000000) {
		  ...
		  stringBuffer.append(getTwoPlacesInWords(wholePart / 100000)).append(
			  " LAKH");
   ...
   * 
   * private static String getMoneyInWords(Integer amount) { return
   * getMoneyInWords(null, amount); }
   */
  /**
   * 
   */
