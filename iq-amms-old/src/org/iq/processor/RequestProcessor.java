...
public class RequestProcessor extends BaseProcessor {
   ...
   * @throws Exception 
   */
  public Map<String, Object> executeService(Map input) throws Exception {
    String requestedServiceName = StringUtil.getStringValue(input.get(REQUESTED_SERVICE_NAME_KEY));
    System.out.println(REQUESTED_SERVICE_NAME_KEY+" = "+requestedServiceName);
    
    Class serviceClass = Class.forName(getServiceClassName(requestedServiceName));
    BaseService service = (BaseService)serviceClass.newInstance();
    
    Set paramNames = input.keySet();
    for (Object paramName : paramNames) {
      System.out.println("Param Name:" + paramName + " and Param Value:" + input.get(paramName));
    }

    HashMap<String, Object> resultMap = service.executeService((HashMap<String, Object>)input);
