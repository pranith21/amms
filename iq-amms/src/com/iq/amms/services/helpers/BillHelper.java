...
public class BillHelper extends BaseHelper {
  ...
  private static String BILL_STATUS_BY_FLAT_ID = "SELECT BILL_STATUS FROM BILLS_INWARDS_MASTER" +
  		" WHERE FLAT_ID = ? ORDER BY DATE_OF_GENERATION DESC LIMIT 1";
  ...
  public void generateInvoices(Date current) throws Exception {
	  ...
	  for (FlatDetailsVO flatDetailsVO : flatDetailsVOList) {
		  ...
		  int latePaymentDisplayDate = Integer.valueOf(apartmentMasterParamsHelper.getParam(ApartmentMasterParamsHelper.LATE_PAYMENT_DISPLAY_DATE));
		  billMasterVO.setDateOfGeneration(current);
		  billMasterVO.setDateOfDue(systemHelper.calculateDate(latePaymentDisplayDate));
  ...
  public void processLatePayments() throws DbException, IOException{
	  FlatsHelper flatsHelper = new FlatsHelper();
	  ...
	  FinancialsHelper financialsHelper = new FinancialsHelper();
	  ApartmentMasterParamsHelper apartmentMasterParamsHelper = new ApartmentMasterParamsHelper();
	  Double latePaymentMinBalance = Double.valueOf(apartmentMasterParamsHelper.
			  getParam(ApartmentMasterParamsHelper.LATE_PAYMENT_MINIMUM_BALANCE));
  ...
  public int getBillStatusByFlatId(int flatId) throws DbException{
	  DataSet dataSet = mySqlSession.executeQuery(BILL_STATUS_BY_FLAT_ID,flatId);
	  if(dataSet != null && dataSet.getRowCount() > 0){
		  return dataSet.getIntValue(0, 0);
	  }
	  return 0;
  }	  
}