...
<%
   	Boolean callFromFinc = Boolean.valueOf(request.getParameter("callFromFinc"));
...
%>
          <table id="tablesTable" class="tablesorter">
          	<thead>
				...
				<th>Owner</th>
				<th>Contact</th>
				<%if(callFromFinc){%><th>Bill Status</th><%}%>
				<th>Current Due</th>
			  </tr>
			</thead>
			<tbody>
			  ...
			  <tr>
			    ...
			    <%
			    	if(callFromFinc) {
			    		...
			    		BillMasterVO billMasterVO = billHelper.getBillMaster(StringUtil.getStringValue(flatsVO.getFlatId()));
			    ...
			    <td><%=status%></td>
			    <%}%>
			    <td><%=flatsVO.getCurrentBalance()%>&nbsp;&nbsp;<a href="bill-details.jsp?flatId=<%=flatsVO.getFlatId()%>">[More...]</a></td>
			  </tr>
<%
		  }
%>
